...
from models.util import ConvReg, SelfA, SRRL, SimKD, mfakd
from dataset.cifar100 import get_cifar100_dataloaders, get_cifar100_dataloaders_sample
from dataset.tinyimagenet  import get_imagenet_dataloader, get_dataloader_sample
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar100',
                        choices=['cifar100', 'tiny-imagenet'], help='dataset')
    parser.add_argument('--model_s', type=str, default='')
    parser.add_argument('--path_t', type=str, default=None,
                        help='teacher model snapshot')

    # distillation
    parser.add_argument('--trial', type=str, default='', help='trial id')
    parser.add_argument('--kd_T', type=float, default=4, help='temperature for KD distillation')
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity', 'vid',
                                                                         'crd', 'semckd', 'srrl', 'simkd', 'mfakd'])
    parser.add_argument('-c', '--cls', type=float, default=0, help='weight for classification')
    parser.add_argument('-d', '--div', type=float, default=0, help='weight balance for KD')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    elif opt.distill == 'mfakd':
        s_n = feat_s[-2].shape[1]
        t_n = feat_t[-2].shape[1]
        model_simkd = mfakd(s_n=s_n, t_n=t_n, factor=opt.factor)
        criterion_kd = nn.MSELoss()
        module_list.append(model_simkd)
        trainable_list.append(model_simkd)
    else:
